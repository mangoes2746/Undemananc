...
export async function getRestaurantPhotos(restaurantId) {
  const q = query(
...
export async function addRestaurantPhoto(uid, userName, restaurantId, url) {
  await addDoc(collection(db, "restaurantPhotos"), {
    uid,
    userName,
    restaurantId,
    url,
...
export async function saveRestaurant(restaurant) {
  const id = restaurant.id || slugify(restaurant.name);
  await setDoc(doc(db, "restaurants", id), serializeRestaurant({ ...restaurant, id }), { merge: true });
  return id;
}

export async function createPartnerRequest(uid, data) {
  const id = `${uid}_${slugify(data.restaurantName || "restaurant")}`;
  await setDoc(doc(db, "partnerRequests", id), {
    uid,
...
export async function seedRestaurants(restaurants) {
  ...
  restaurants.forEach(r => {
    ...
    batch.set(ref, serializeRestaurant(r), { merge: true });
  });
  await batch.commit();
}
